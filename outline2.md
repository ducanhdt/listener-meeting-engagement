...
* **Key Findings:** 1. Dyadic listener $\leftrightarrow$ speaker behavioral alignment is the single strongest predictor of engagement, improving Random Forest macro-$F_1$ from 0.341 to 0.378.
    ...
    4. Unsupervised clustering ($k=2$) fails to recover engagement labels ($\text{ARI} \approx 0$). The middle *Intermittent engagement* class — a genuine but unstable state — is difficult to recover from single 5-second windows, whereas a binary active-vs-rest reduction yields a highly learnable signal ($\text{macro-}F_1 \approx 0.63$).
...
* **A Change of Perspective (Interactional Engagement):** Rather than treating the listener as a solo signal source, we shift the analytical perspective to the listener's relation with others in the meeting. A silent listener's state is relational—manifested through behavioral alignment (mimicry/entrainment) with the group and through co-variation with the concurrently active speaker's nonverbal and prosodic behavior.
...
* **Core Contributions:**
    ...
    3. *Construct Analysis:* Provide empirical evidence that the 3-class engagement task collapses toward a binary one in this setting, because the middle *intermittent* class is defined by temporal fluctuation that single static windows cannot capture.
...
* **Positioning:** We explicitly close the gap between isolated listener tracking and social synchrony by evaluating the direct, windowed listener $\leftrightarrow$ speaker relation within a supervised framework, moving beyond purely descriptive or unvalidated clustering approaches.
...
* **Label Space:** Primary target is 3-class: *Active Engagement*, *Intermittent Engagement* (partial engagement that may show occasional distraction while still following the speaker), and *Disengagement*.
...
* **The Reliability Constraint:** Label scarcity and noise shape our framework. Inter-rater agreement is modest (Fleiss' $\kappa = 0.42$). The middle "Intermittent Engagement" class—partial, unstable engagement that may show occasional distraction while still following the speaker—is the hardest to judge from a single short window and concentrates the disagreement. Filtering for strict agreement yields 89 gold-labeled windows (44 Active, 28 Intermittent, 17 Disengaged).
...
* Descriptive class means show a distinct vocal trend: windows with **Active** listeners co-occur with speaker rising intonation ($+1.5$ st), **Intermittent-engagement** listeners with higher rises ($+2.6$ st), and **Disengaged** listeners with flat or falling tones ($-3.6$ st).
...
* **The Convergence Argument:** Three lines of evidence indicate the 3-class task collapses toward binary in this setting: (1) Construct nature (the middle *intermittent* class is a genuine but temporally unstable state that 5-second static windows cannot represent), (2) Unsupervised topography ($k=2$), and (3) Class learnability (Active vs Rest yields a robust $+0.30$ improvement over the baseline floor).
